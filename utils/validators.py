...
class GenerationValidator:
    ...
    def validate_image_size(cls, model_name: str, img_size: Union[int, str]) -> None:
        ...
        if (
            img_size_int < config["min_img_size"]
            or img_size_int > config["max_img_size"]
        ):
            raise HTTPException(
                status_code=400,
    ...
    def validate_generation_params(
        ...
        if guidance_scale is not None:
            ...
            if (
                guidance_scale_float < 0
                or guidance_scale_float > cls.MAX_GUIDANCE_SCALE
            ):
                raise HTTPException(
                    status_code=400,
        ...
        if num_inference_steps is not None:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Number of steps must be an integer"
                )
